        ...
        <?php else : ?>
            ...
            <div class="spux-layout">
                <section class="spux-main" aria-labelledby="spux-section-title">
                    ...
                    <?php else : ?>
                        <div class="spux-card spux-prose">
                            <h2 id="spux-section-title"><?php echo esc_html((string) ($sections[$section] ?? __('Overview', 'sabri-public-experience'))); ?></h2>
                            <?php if (in_array($section, ['overview', 'about'], true) && ! empty($profile['bio'])) : ?>
                                <div><?php echo wp_kses_post(wpautop((string) $profile['bio'])); ?></div>
                            <?php elseif ($section === 'overview' && ! empty($profile['headline'])) : ?>
                                <p><?php echo esc_html((string) $profile['headline']); ?></p>
                            <?php else : ?>
                                <div class="spux-state spux-state--empty" role="status">
