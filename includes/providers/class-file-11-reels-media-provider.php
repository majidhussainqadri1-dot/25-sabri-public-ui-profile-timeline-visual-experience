...
/** Read-only File 11 Reels adapter over File 10's canonical video objects. */
final class File_11_Reels_Media_Provider implements Profile_Section_Provider
{
    ...
    {
        $file_10 = defined('SVW_VERSION') ? trim((string) SVW_VERSION) : '0.0.0';
        if (! self::version_supported($this->get_version(), self::MINIMUM_VERSION, self::MAXIMUM_VERSION)
            || ! self::version_supported($file_10, self::FILE_10_MINIMUM, self::FILE_10_MAXIMUM)
            || ! class_exists('SVW_Helpers')
            || ! defined('SVW_Helpers::TYPE')) {
            return false;
        }
        if ((string) \SVW_Helpers::TYPE !== 'svw_video') {
            return false;
        }
    ...
    public function query(int $user_id, array $profile, array $context = []): array
    {
        ...
        foreach (array_slice($posts, 0, $limit) as $post) {
            ...
            if (! self::public_post_matches($post, $id, $user_id) || (string) \SVW_Helpers::meta($id, 'is_reel', '0') !== '1') {
                continue;
            }
    ...
    private static function version_supported(string $version, string $minimum, string $maximum): bool
    {
    ...
    private static function reels_label(): string { return function_exists('__') ? __('Reels', 'sabri-public-experience') : 'Reels'; }
    private static function watch_label(): string { return function_exists('__') ? __('Watch Reel', 'sabri-public-experience') : 'Watch Reel'; }
