...
final class Design_System
{
    ...
    public function body_classes(array $classes): array
    {
        ...
        $classes[] = function_exists('is_rtl') && is_rtl() ? 'sabri-visual-direction-rtl' : 'sabri-visual-direction-ltr';
        $classes[] = self::shell_is_available() ? 'sabri-visual-shell-connected' : 'sabri-visual-shell-degraded';
    ...
    public static function should_enqueue(): bool
    {
        ...
        foreach (['is_feed', 'is_embed', 'is_robots', 'is_favicon', 'is_trackback'] as $conditional) {
            if (function_exists($conditional) && $conditional()) { return false; }
        }
        return true;
    }
    ...
    public static function contract(): array
    {
        return [
            ...
            'runtime_version' => defined('SABRI_PUBLIC_EXPERIENCE_VERSION') ? (string) SABRI_PUBLIC_EXPERIENCE_VERSION : '',
            ...
            'file_20_integration' => self::file_20_contract(),
            ...
            'optional_sections' => [
                ...
                'internal_projection_key' => [
                    'field' => 'projection_key','format' => 'sha256','rendered_publicly' => false,
            ...
            'profile_presentation' => [
                ...
                'doctor_contact_requires_file_03_consent' => true,'right_sidebar_requires_real_content' => true,'breadcrumb_html' => true,
                'breadcrumb_schema' => true,'missing_profile_default_status' => 404,'explicit_tombstone_status' => 410,
                'doctor_structured_data_type' => 'Physician','profile_qr_image_filter' => 'sabri_public_experience/profile_qr_image_url',
                'profile_qr_same_site_only' => true,'profile_qr_tracking_free_required' => true,'profile_qr_hidden_when_unavailable' => true,
            ],
            'latest_governing_requirements' => [
                'file_specific' => ['F25-CEN-01', 'F25-CEN-02'],'acceptance_journeys' => ['AJ-04', 'AJ-31', 'AJ-32', 'AJ-33', 'AJ-39', 'AJ-40'],
                'global_search_discovery_ranking_owner' => 'file-26','matrix' => 'config/central-2026-file25-requirements.json',
            ...
            'staging_package' => [
                'contract_version' => '1.0.0','builder' => 'tools/build-staging-package.php','dependency_matrix' => 'config/staging-dependencies.json',
                'package_root' => 'sabri-public-experience','manifest' => 'STAGING-MANIFEST.json','checksum' => 'sha256','deterministic_source_date_epoch' => true,
                'development_files_excluded' => true,'staging_acceptance_implied' => false,'production_acceptance_implied' => false,
            ],
            'asset_handle' => 'sabri-visual-design-system','css_prefix' => 'sabri-ui-','duplicates_file_20_shell' => false,'creates_file_26' => false,
        ];
    }
    ...
    public static function filter_shell_visual_contract(mixed $contract): array
    {
        unset($contract);
        return [
            'owner' => 'file-25','version' => self::CONTRACT_VERSION,
            'tokens' => [
                'primary_color' => '#087A4E','background' => '#f7f5f1','surface' => '#ffffff','surface_strong' => '#E7F5EE',
                'text' => '#171717','muted' => '#5f6368','border' => '#dfe2e6','focus' => '#0b57d0','radius' => 16,
                'font_scale' => 1.0,'density' => 'comfortable','shadow' => '0 6px 24px rgba(17,17,17,0.10)',
            ],
        ];
    }
    ...
    {
        return array_merge(is_array($tokens) ? $tokens : [], self::tokens());
    }

    /** @param mixed $components @return array<string,mixed> */
    public static function filter_components(mixed $components): array
    {
        return array_merge(is_array($components) ? $components : [], Components::contract());
    }

    private static function shell_is_available(): bool
    {
        return class_exists(File_20_Integration::class) && File_20_Integration::is_compatible();
    }

    /** @return array<string,mixed> */
    private static function file_20_contract(): array
    {
        return class_exists(File_20_Integration::class)
            ? File_20_Integration::contract()
            : [
                'owner' => 'file-20',
                'reviewed_minimum_version' => '1.4.12',
                'reviewed_maximum_version_exclusive' => '1.5.0',
