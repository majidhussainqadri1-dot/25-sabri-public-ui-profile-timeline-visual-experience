...
Production-oriented WordPress module for the **Sabri Social Homeopathy Platform**. This is the same File 25; no duplicate File 26 is created.

## Current source scope

The Draft PR contains governance and safety, File 20 shell-token integration, Founder/Doctor profile foundations, the federated File 21 timeline, reusable global visual components, governed optional profile sections, native File 06 Knowledge, File 10/11/12 Media, File 18 Marketplace projections, exact File 24 integration, File 03 contact-consent enforcement, structured public section APIs, artifact-integrity visual acceptance, deterministic staging release engineering, and installed-candidate preflight.
...
- File 00/03 remain identity/profile authorities and File 03 remains public-contact consent owner;
- verified Doctor status no longer bypasses explicit contact consent;
- canonical Founder public spelling is immutable in presentation code;
- profile and OpenGraph media are same-origin only, with privacy-safe initials instead of silent Gravatar fallback;
- Doctor profiles default to two-column shell layout and use a third column only when real sidebar content is declared;
- visible and structured breadcrumbs, safe `og:image:alt`, and a defined 404/410 missing-profile policy;
- reusable `sabri-ui-*` components, states, notices, forms, tables, and content cards;
- one allow-listed card normalizer shared by HTML and REST;
- structured public Knowledge and Media endpoints plus aggregate provider health without provider/native identifiers;
...
- **File 03:** profile master data, local profile media, and explicit public-contact consent.
...
- **File 25:** public visual projection, profile navigation, reusable components, responsive refinement, accessibility, SEO presentation, visual acceptance, and staging-candidate packaging.

File 25 does not create incidents, write File 24 audit records, process privacy exports/erasures, manage findings, approve contacts, accept risks, or claim native-content ownership.
...
- Runtime confirmation of File 03 contact-consent changes and File 25 public projection behavior.
