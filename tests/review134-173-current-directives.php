...
$assert(str_contains($css, '--sabri-visual-warning') === false || str_contains($css, '--sabri-visual-primary'), 'Review 137: primary override does not replace semantic status tokens.');
$assert(($filtered['tokens']['color-primary']['fallback'] ?? '') === '#15803d', 'Review 138: current token amendment has contract precedence.');
...
$assert(str_contains($back, 'arrow') === false && str_contains($back, '>Back<') && str_contains($back, 'href="https://example.test/previous/"'), 'Review 149: Back control is same-site and explicitly labelled.');
...
$assert(str_contains($back, 'sabri-ui-icon-button__label') && str_contains($back, '<svg'), 'Review 154: icon and visible text are paired.');
...
$assert(str_contains($badge, 'title="Owner-supplied governed tier."'), 'Review 165: ranking explanation is accessible.');
...
$assert(! str_contains($downloadUnknown, 'href=') && str_contains($downloadUnknown, 'disabled'), 'Review 167: unknown owner eligibility fails closed.');
...
$assert(str_contains($revoked, 'sabri-ui-download-reason') && str_contains($revoked, '<svg') && str_contains($revoked, 'aria-describedby'), 'Review 171: unavailable state is not color-only.');
...
$assert(($contract['range'] ?? '') === '134-173' && ($contract['count'] ?? 0) === 40 && ($contract['hostinger_staging_accepted'] ?? true) === false && count($contract['rounds'] ?? []) === 40, 'Review 173: forty rounds recorded without external acceptance overclaim.');
