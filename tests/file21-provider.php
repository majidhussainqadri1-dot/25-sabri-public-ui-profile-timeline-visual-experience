...
namespace {
    ...
    $check(count(\Sabri\HomeNewsFeed\ProfileTimeline::$calls) === 3, 'Provider must page through File 21 in bounded native chunks.');
    $check(
        \Sabri\HomeNewsFeed\ProfileTimeline::$calls[0]['page'] === 1
        && \Sabri\HomeNewsFeed\ProfileTimeline::$calls[2]['page'] === 3,
        'Native File 21 pages must be requested in order.'
    );
    $check($items[0] instanceof \Sabri\PublicExperience\Normalized_Timeline_Item, 'Provider must return normalized File 25 timeline items.');
    ...
    $check(
        array_column(\Sabri\HomeNewsFeed\ProfileTimeline::$calls, 'per_page') === [20, 20, 20],
        'Native File 21 page size must remain stable so offsets cannot overlap.'
    );
    ...
    $check(\Sabri\HomeNewsFeed\ProfileTimeline::$calls === [], 'Unsupported content filters must not query File 21.');

    \Sabri\HomeNewsFeed\ProfileTimeline::$disabled = true;
    ...
    \Sabri\HomeNewsFeed\ProfileTimeline::$disabled = false;
